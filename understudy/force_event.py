...
from understudy.teyvat_ids import (  # noqa: F401  (re-export; see EB-396)
    _CAMEL, _SUBSTITUTION_ROW, _SUBSTITUTIONS_SOURCE, dressed_to_base,
    resolve_event_id, wire_id)
...
# ------------------------------------------- EB-767: dressed -> base ------
#
# THE TABLE AND THE ID RULE ARE `understudy/teyvat_ids.py` and are re-exported
# here whole, so `force_event.wire_id`, `force_event.dressed_to_base` and
# `force_event.resolve_event_id` are the names they always were. They moved
# under `EB-396`, which needed the same translation on the blind page: this
# module imports `tier05.maps` and `policy_v0`, and `blindplay` may have
# neither. The whole rationale -- why the op takes base ids only, and why the
# table is the generated file itself rather than a copy -- travelled with the
# code and is in that module's own comments.
