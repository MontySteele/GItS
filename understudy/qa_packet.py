...
def _copy_spark_price(card_id: Any, upgraded: bool,
                      sparks: dict[str, int]) -> int | None:
    """`spark_price_for` over an already-built printed index."""
    if upgraded:
        up = upgraded_spark_price(card_id)
        if up is not None:
            return up
    return sparks.get(card_key(card_id))


def spark_price_for(card_id: Any, upgraded: bool,
                    repo: Path | None = None) -> int | None:
    """The Spark price THIS COPY charges: the upgraded price on an upgraded
    copy whose upgrade moves it, the printed one otherwise, `None` where the
    card prints none.

    THE INDEX ABOVE IS KEYED ON THE CLASS and reads the BASE branch, so on its
    own it priced Sparkling Burst+ in hand at the 2 Sparks the unupgraded card
    charges. The copy's own `upgraded` flag is on the wire, so the page asks
    for the copy's price rather than the class's.
    """
    if upgraded:
        up = upgraded_spark_price(card_id, repo)
        if up is not None:
            return up
    return printed_spark_index(repo).get(card_key(card_id))


#: An energy cost the upgrade MOVES: `EnergyCost.UpgradeBy(n)` in `OnUpgrade`,
#: which is the one spelling the codegen and the hand-written cards use.
_ENERGY_UPGRADE_RE = re.compile(r"EnergyCost\.UpgradeBy\(\s*(-?\d+)\s*\)")


@lru_cache(maxsize=4)
def _energy_upgrade_index_cached(repo: Path) -> tuple[tuple[str, int], ...]:
    """`{card id: the energy delta its upgrade applies}`, non-zero only."""
    index: dict[str, int] = {}
    root = repo / "klee-mod"
    if not root.is_dir():
        return ()
    for path in sorted(root.glob(_CARD_SOURCE_GLOB)):
        try:
            ...
            continue
        delta = sum(int(m) for m in _ENERGY_UPGRADE_RE.findall(src))
        if not delta:
            continue
        key = _class_key(src)
        if key:
            index.setdefault(key, delta)
    return tuple(sorted(index.items()))


def upgraded_energy_delta(card_id: Any, repo: Path | None = None) -> int:
    """How much this card's upgrade moves its energy cost, 0 where it does
    not."""
    root = repo if repo is not None else Path(__file__).resolve().parents[1]
    return dict(_energy_upgrade_index_cached(root)).get(card_key(card_id), 0)


def _upgrade_moves_cost(card_id: Any, repo: Path | None = None) -> bool:
    return (upgraded_spark_price(card_id, repo) is not None
            or upgraded_energy_delta(card_id, repo) != 0)
...
def upgraded_cost_label(card: dict[str, Any], card_id: Any,
    ...
    AN ENERGY COST THE UPGRADE CUTS IS THE SAME FIND (Salon Debut, 1 to 0):
    the energy half of the slot moves by the `EnergyCost.UpgradeBy` delta,
    read off the number the Smith is showing, and prints the same way.
    """
    spark_up = upgraded_spark_price(card_id, repo)
    energy_delta = upgraded_energy_delta(card_id, repo)
    if spark_up is None and not energy_delta:
        return ""
    moved = dict(card)
    if spark_up is not None:
        moved["printed_spark"] = spark_up
    if energy_delta:
        shown = _text(card.get("cost"))
        base = (int(shown) if shown.lstrip("-").isdigit()
                else card.get("printed_cost"))
        if not isinstance(base, int):
            # No number to move: the slot cannot be written without a guess.
            return ""
        moved["cost"] = str(max(0, base + energy_delta))
    return cost_label(moved)
...
# `EB-529`. THE UPGRADE THAT IS A CLAUSE RATHER THAN A NUMBER.
#
# THE FIND (Furina r12 lane 2). "The upgrade screen showed no upgrade at all
# for Aria of Recompense, Salon Debut, An Invitation and Fischl -- Oz, with no
# line saying why, while every other card printed an `Upgraded:` preview."
#
# THREE OF THE FOUR ARE ONE SHAPE. `{IfUpgraded:show:A|B}` prints arm B on the
# card as it stands and arm A once it is upgraded -- `Gain {IfUpgraded:show:8|5}
# Encore`, `... to your hand{IfUpgraded:show:, free this turn|}`,
# `{IfUpgraded:show:Draw 1 card.|}` -- and where NEITHER arm holds a brace, that
# is a swap this module can make exactly: the pattern reads arm B off the
# printed face and the render writes arm A. No arithmetic, no guess.
#
# THE FOURTH IS THE ONE THAT CANNOT BE. Salon Debut's arm carries a hole of its
# own (`{IfUpgraded:show:Gain {Encore:diff()} [gold]Encore[/gold].|}`), which is
# two sentences rather than one sentence with a number in it. That row gets a
# REASON instead, which is the other half the seat asked for: "no line saying
...
_PIECE_RE = re.compile(
    _IF_UPGRADED_RE.pattern
    + r"|\{([A-Za-z_][A-Za-z0-9_]*)(?::([^{}]*))?\}")

#: Why a row prints no upgraded face, in the player's own vocabulary. Each is
#: a fact about THIS card rather than about this module: a reader who is
#: deciding what to spend a Smith on can act on all four.
...
def _hole_deltas(template: str,
                 deltas: dict[str, int]) -> dict[str, int] | None:
    ...
    for name, delta in deltas.items():
        ...
        # moves no number at all" are two different answers and the second one
        # can still have a clause to swap.
        return None
    return out
...
def _upgraded_face_index_cached(repo: Path) -> tuple[
    ...
    all ... with no line saying why" -- each carry one of `NO_PREVIEW_*`
    instead of nothing. A row with a reason has no template to render and a
    row with a template has no reason; never both.
    """
    index: dict[str, tuple[str, tuple[tuple[str, int], ...], str]] = {}
    root = repo / "klee-mod"
    if not root.is_dir():
        return ()
    for path in sorted(root.glob(_CARD_SOURCE_GLOB)):
...
def upgrade_preview(card_id: Any, printed: str,
    ...
    if reason == NO_PREVIEW_NO_NUMBER and _upgrade_moves_cost(card_id, root):
        # The upgrade moves the COST -- a Spark price or an energy cost -- and
        # nothing in the sentence: the upgraded face is this one, and the
        # caller prints the new cost in front of it (`upgraded_cost_label`),
        # the way the head line prints the current one.
    ...
    # showing; a plural hole reads whichever arm it printed; a two-arm swap
    # (`EB-529`) reads the UNUPGRADED arm, which is what the card in front of
    # the player is showing, and writes the other one.
    pattern, pieces, at = [], [], 0
    for m in _PIECE_RE.finditer(template):
        pattern.append(re.escape(template[at:m.start()]))
        pieces.append(("literal", template[at:m.start()], ""))
        if m.group(1) is not None:
            if not m.group(2):
                # AN EMPTY UNUPGRADED ARM TAKES THE SPACE IN FRONT OF IT WITH
                # IT. `... a random enemy. {IfUpgraded:show:Draw 1 card.|}`
                # renders unupgraded as a sentence with a trailing space, and
                # the game prints the trimmed one -- so the literal before an
                # empty arm matches with or without its own tail whitespace.
                # The PIECE keeps the space, because the upgraded face needs it
                # between the two sentences.
                pattern[-1] = (re.escape(template[at:m.start()].rstrip())
                               + r"\s*")
            pattern.append(re.escape(m.group(2)))
            pieces.append(("swap", m.group(1), ""))
            at = m.end()
            continue
        spec = m.group(4) or ""
        if spec.startswith("plural:"):
            ...
            pattern.append(r"(-?\d+)")
        pieces.append(("hole", m.group(3), spec))
        at = m.end()
    pattern.append(re.escape(template[at:]))
    pieces.append(("literal", template[at:], ""))
    ...
    for kind, name, spec in pieces:
        if kind == "literal":
            ...
            continue
        if kind == "swap":
            # `EB-529`: the upgraded arm, verbatim. `name` IS that arm -- the
            # pattern above already consumed the other one off the screen.
            out.append(name)
            continue
        if spec.startswith("plural:"):
...
def _hand(state: dict[str, Any], loc: dict[str, str],
    ...
    for entry in (state.get("player") or {}).get("hand") or []:
        ...
        cards.append({
            ...
            # An UPGRADED copy charges its upgraded price (`spark_price_for`).
            "printed_spark": _copy_spark_price(
                entry.get("id"), bool(entry.get("is_upgraded")), sparks),
