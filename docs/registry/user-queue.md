...
| # | Ask | Shape |
|---|---|---|
| ~~10.1~~ | ~~**Five of the nineteen art candidates would fail the art lint if you pick them.** Pick a way forward. `S4-G12` changed shape: five of nineteen art candidates fail `art_lint` if picked; `standing_room_only`'s recommended r3 fails `ART-L1`~~ | **RULED 2026-08-06 — option (a), EXECUTED.** *"let's go with a)"*: `grand_gala` released `Opera Epiclese.png` and the `standing_room_only` overturn landed as its rank 1 (`art/plan.tsv`; lint clean on the collision). `grand_gala` is in the re-hunt: three scratch-verified candidates joined the `S4-G12` sheet so [USER] picks once. The other four failures (`warmup_act` r4, `crowd_work` r2, `audience_participation` r4, `tempo_change` r3) stay as noted per-item in `docs/g12-review-2026-08-05.md` §2 — no ruling owed unless a listed fallback is picked |
