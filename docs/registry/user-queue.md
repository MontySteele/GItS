...
| Q12 | **Errata Batch 2 — PENDING WORK, not an ask.** Eight ratified fixes are queued and none has been written. Listed here so that "ruled" is never mistaken for "shipped" | *(no reply needed)* | Contents, in order: (1) term-3 fanfare credit, sim; (2) `NC-8` potions consumed, sim; (3) `NC-1` companion-power damage through the pipeline, mod; (4) `NC-11` power-sourced block raw, mod; (5) `NC-7` Frozen unified, both; (6) `NC-10` shop slot spec, both; (7) `FLAG-1` accumulator turn-scoped, both; (8) `FLAG-2` (i)/(ii), (ii) sim-only. **After the batch:** parity vectors, all lints, S13 harness characterization on affected lines (report transitions, don't grade), version-stamp questions surfaced in one place. **One implementation question inside it is deliberately open and must not be answered silently:** `NC-10`'s rarity-odds renormalization within the Uncommon+ pool | R113 (item 1), R114 (items 7–8), R116 (items 2–6 + the batch table) |
