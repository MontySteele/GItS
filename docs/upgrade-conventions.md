...
3. **Cost reduction is a rarity privilege.** 92 cost upgrades: 52 rare, 32 uncommon, 2 common. Always −1 (one −2 outlier on a 9-cost). Rule: cost upgrades are the rare-tier "your payoff gets cheaper" move. **Correction (R148, 2026-08-10): "commons never upgrade cost" over-claimed its own data** — the mining above found **2 base-game common cost reductions**, so the base game does it, rarely. The honest statement is *strongly rarity-skewed*, not *never*.
...
- Commons: exactly one number bump per grammar row 1/2. Common cost reductions are the rare exception, not zero — **`shared_billing`'s 1→0 is ALLOWED as-is (R148, 2026-08-10)**, ruled rather than grandfathered. The default stays "don't"; a common cost upgrade wants a reason, and this one has one.
