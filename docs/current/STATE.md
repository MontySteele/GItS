...
|---|---|---|---|
| `C` `CONSTANTS_VERSION` | **6** | `tier0/constants.py` | Frozen-unified duration counter + alpha boss-room freeze scope + shop-slot spec (R117/Q14). |
| `RT` `RUNTEMPLATE_VERSION` | **7** | `tier0/constants.py` | Fixed template retired at v6; each act now generates a real StS2 map. Acts 2–3 event pools + corrected option valuation. |
| `D` `DRAFTER_VERSION` | **14** | `tier0/constants.py` | Generic-limb `core_complete` now requires an on-plan payoff. Held at 14 for the R121 shield (non-bump flagged, not settled). |
| `P` `POLICY_VERSION` | **3** | `tier05/draft.py` | Pilot/drafter policy epoch (R66 character-aware registry). Enters the stamp as `P`. |
...
- **Pilot policy:** `POLICY_VERSION = 3` lives in `tier05/draft.py` and enters
