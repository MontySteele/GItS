...
absent on a fresh clone. **On this machine that tree is currently EMPTY —
destroyed 2026-08-24, the fourth such loss — so `real_ironclad` and
`real_silent` cannot be loaded at all and any experiment naming them stops at
`KeyError`. It is not recoverable from git or from any copy on this machine.**
See BACKLOG `EB-128`.
...
- **EB-118 richness pass** — Phase-0 contract in HEAD
  ...
  **R203 adopted** the Explosives Workshop conversion
  ...
  and `ref_ironclad` — printed their prior values on every column. The
  instrument finding that post-read raised is a [USER] call and is now `QUEUE`
  `M42`: `random_enemies` sits in BOTH `RANDOM_TARGETS` and `MULTI_TARGETS`,
  so de-randomizing a placement row deletes an `enemy_count` shared read along
  with the randomness, and repairing it means a classifier `VOCAB_VERSION` v4
