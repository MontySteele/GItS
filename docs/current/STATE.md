...
|---|---|---|---|
| `C` `CONSTANTS_VERSION` | **9** | `tier0/constants.py` | Slot-2 rarity floor restored ([USER] 2026-08-10, S4-G10 close-out): the shop's wildcard companion slot rolls Uncommon-or-better in **both** engines. Every §4.7 shop number taken under C6–C8 is archive. The window includes the `exp_shop_companion_channel` instrument repairs and the X7/X8 rarity promotions (R161, R162 — `friendly_visit`, `chain_fuse`, `careful_arrangement` Common → Uncommon); further errata may join C9 until a number is quoted under it. |
| `RT` `RUNTEMPLATE_VERSION` | **10** | `tier0/constants.py` | R82 reopened ([USER] 2026-08-10, M7): the five enchant events join the pools, so event-pool odds move in **every** act for **every** character and the enchantment vocabulary enters the run layer. Enchantments are post-draft only, so `D` and `P` are untouched. v9 event numbers do not carry across. |
...
| `P` `POLICY_VERSION` | **7** | `tier05/draft.py` | R176: the pilot values `copy_companion_in_hand` / `replay_next_companion` — every Klee tier0.5 number moves; the payoff-reach `DRAFTER_VERSION=14` pin is untouched. |
