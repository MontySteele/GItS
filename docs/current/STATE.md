...
step (iv) makes Window 1's pre-state. None of `C15`, `C16`, `C17` or `C18`
moves the table: no standing baseline is owed at any of those bumps, because no
pending decision names one. `C16`, `C17` and `C18` do move the table further
OUT OF DATE — between them all three characters' arms are archive in combat as
well as in drafting, and `C18` adds the two `real_*` floor rows and the ANCHOR
ITSELF, whose combat behaviour moved even though it renormalises to 3.0 — and
the read that repairs that is `W3`'s single public one, not a table taken per
window. **`C19`/`D17`/`P10` is that window and it is now LANDED, so the read it
owes is the one outstanding item on this baseline** — ONE published table, and
under R211 item 7 it publishes **DIAGNOSTIC-SCOPED rather than as the Phase-4
milestone table**, for three reasons already known and named at the `C19`
constant. Per-character attribution inside `W3` is by commit-hash scratch
comparison (R207) and is not citable the way a stamped table is.
...
- **Furina** — **83 of 84** generated, 1 blocked
  ...
  new blocker.
- **Two cards ship with NO UPGRADE PATH in the mod, and it is named rather
  than tolerated** — `hold_the_line` (klee) and `take_it_from_the_top`
  (furina), whose `W3`-ratified `{conditional_block: +3}` and
  `{conditional_damage: +4}` deltas tier0 applies correctly and
  `gen_klee_cards.EXPRESSIBLE_DELTAS` cannot emit. A real two-engine
  divergence on a player-facing choice, carried as curated debt in
  `tools/lint_upgrade_coverage.CODEGEN_DEBT` and
  `tier0/tests/test_roster_codegen.FURINA_UPGRADE_GAP_PENDING_FB1`, gated on
  BACKLOG `EB-140`. The deltas were deliberately NOT re-ruled into keys the
  emitter happens to have.
...
- **`EB-118` richness pass — Phase 2 is COMPLETE and every Phase-3 content
  window is LANDED, W3 included. What the row still owes is ONE READ.** All
  three Phase-2 windows are closed: the
  ...
  read was registered for `C16` or `C17`, and none is invented. **Window 3 (the
  R211 slate) LANDS as ONE public window at `C19`/`D17`/`P10`** with
  `PILOT_WEIGHTS_VERSION` 5 beside it — eight sheet rows across all three
  characters, one new drafter dial, one new priced condition, and one pilot
  chooser repair. Two of its numbers are **[USER]-overridable and each lives in
  exactly one constant**: `STATIC_SPARK_SPEND_COST = 2.5` and
  `STATIC_SPOTLIGHT_MOVED_SHARE = 0.167`, both in `tier05/draft.py`. **WHAT IT
  STILL OWES IS THE SINGLE STANDING READ**, which publishes DIAGNOSTIC-SCOPED
  and is NOT the Phase-4 milestone table (R211 item 7); per-character
  attribution inside the window is commit-hash scratch comparison (R207) and is
  not citable. Its two ruled scope deferrals leave with it: the Klee bomb-board
  readers to `W10`, and `F3`/the Encore-spender family to `W11`, which opens
  only after the pilot's Encore opportunity-cost repair. One thing in the slate
  could not be built as ratified — the generator cannot emit
  `conditional_block` or `conditional_damage` upgrade deltas — and it is filed
  as `EB-140` rather than answered by re-ruling either delta.
