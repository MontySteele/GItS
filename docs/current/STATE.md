...
- **EB-118 richness pass** — Phase-0 contract in HEAD
  ...
  **PHASE 2A IS LANDED AND ITS WINDOW IS CLOSED, 2026-08-24 — `POLICY_VERSION`
  ...
  no `RT`, `D` or `C` beside it, so the live cell is now **`RT12/D16/P8/C13`**.
  ...
  **THE `C` AND `D` WINDOWS ARE CLOSED, 2026-08-24 — `CONSTANTS_VERSION` 13
  ...
  those windows closed at was `RT12/D16/P7/C13`; `P` has since moved to 8 at
  the 2A landing above, so the live cell is `RT12/D16/P8/C13` and the
  re-baseline below is a `P7` reading. C13 declares ONE window over every material
  ...
  `QUEUE` `M41`. **What those windows left open was ACTIVATION, and HALF OF IT
  IS NOW CLOSED:** `PILOT_POLICIES_ENABLED` is `True` and `P` is 8 (the 2A
  landing above); `MODE_CHOOSER_ENABLED` is still `False` and 2C keeps its own
  window and its own `P` bump (R191). **The re-baseline is published** —
  ...
  **The sequence is five steps and W1 is step (v):** (i) rule the three Phase-1
  ...
  their single re-baseline), and **2A's activation window closed after them**
  on its own `P` bump (`POLICY_VERSION` 7 → 8, `PILOT_WEIGHTS_VERSION` 2 → 3).
  What remains of this step is **2C's mode-chooser window alone** —
  `MODE_CHOOSER_ENABLED` still `False`, its own `P` bump reserved — and then
  the Phase-2 post-read, which is owed once over both activation windows
