...
| `C` `CONSTANTS_VERSION` | **20** | `tier0/constants.py` | `EB-139`'s Swirl aura-aware bind (R211): a card carrying an aimed Swirl binds whole to the lowest-HP aura-bearer, which moves one companion's damage and does not move the anchor. History → [`STAMPS.md`](STAMPS.md). |
