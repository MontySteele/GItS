# Register ids — minting, retiring, and why the ceiling is derived
...
1. **Every defined id is unique** — no two rows in one register, and no id
   defined in both registers at once.
2. **No retired number is re-minted** — a row that closes leaves HEAD
   (CLAUDE.md §Norms), so its number is invisible to every other check in the
   tree. `RETIRED` in the lint is the only thing that remembers it.
3. **Every number below a series' ceiling is accounted for** — it defines a
   row, or it is in `RETIRED`. A hole is a close that was never written down.

## What changed on 2026-09-02: the ceiling is DERIVED

It used to be a literal — `CEILINGS = {"EB": 317, "M": 69}` — bumped by hand in
the same commit as the row that minted past it. The rule was sound and the line
was a bottleneck: **every branch that minted an `EB` row edited the same line of
the same file**, and a single sitting of twelve parallel agents produced four
conflicts on it, each resolved by hand.
...
computed at import. **Minting edits the register and nothing else**, so two
branches minting in parallel merge clean.

**The collision hazard did not vanish, it moved.** Two branches that each take
"the next free number" against the same base still take the *same* number. That
used to surface as a merge conflict in the lint file, resolved by a person; it
now surfaces as guarantee 1's `DUPLICATE` finding on the merged tree, in CI,
with both rows named and both line numbers printed. A gate that fires is a
better place for it than a conflict a hand resolves — and `tools/mint_row.py`
prints the id it took, so the second branch sees it in its own diff.

## Minting a row

```sh
python tools/mint_row.py BACKLOG "tools — codegen, lint, scripts, refactors" \
    --scope "..." --next-action "..." --gate "..." --acceptance "..." \
    --provenance "..." --write
```

It derives the next free number, prints the row and its character count against
the 600-char shape gate, inserts it at the top of the named section's table, and
runs both register lints. Nothing in `tools/lint_register_ids.py` is edited.

An id can be forced with `--id EB-320` — for a packet that reserved a number, or
a re-run that must land on the number it printed last time.

## Retiring a row (the successor to "LEFT `OPEN_IDS`")

When a row closes and leaves HEAD, **in the same commit**:

1. delete the row from its register;
2. add its number to `RETIRED[<series>]` in `tools/lint_register_ids.py` —
   extending a span (`(213, 219)` becomes `(213, 220)`) or appending the bare
   integer;
3. write the one-line note in the ledger below, saying what the row was and on
   what acceptance it closed.

Guarantee 3 is what makes step 2 non-optional: skip it and the number sits below
the derived ceiling, defines no row and is in no set, which is exactly the hole
the next mint falls into. The lint prints `UNRECORDED RETIREMENT: EB-<n>` until
it is recorded. That is the same self-enforcing shape `OPEN_IDS` had — the
retirement is recorded by the act of retiring — moved to the other side.

## Skipping a number another branch took

Sometimes you know a parallel branch has taken `EB-<n>` and you want the one
after it. Your branch does not hold that row, so `<n>` is a hole and the lint
says so. **Record it in `RETIRED` on your branch, with a note saying where it
went** — *minted <date> on `<branch>`; the ceiling is a floor on "ever issued"
and only moves forward, so it carries the number whether or not this branch
holds its row.* That sentence is the practice the old ceiling comments already
followed (`EB-285..EB-291` and `EB-296..EB-300` in the ledger below); the
mechanism moved, the practice did not.

The alternative is to let both branches take the same number and settle it at
the merge, where guarantee 1 names both rows and both line numbers. Either is
legitimate: the second is cheaper when the branches are hours apart and worse
when they are days.
...
`tools/lint_r_numbers.py` owns them, reading `## R<n>` headings rather than
table cells, and its `R_CEILING` is still a hand-bumped literal — deliberately,
because a ruling number is issued by a commit message and there is no row to
derive it from. A register row defining an `R` number is refused by name
(`FOREIGN SERIES`), because it would sit outside both guards.

---

# The issued-id ledger

Everything below is the per-id record, **moved verbatim on 2026-09-02** out of
the comments in `tools/lint_register_ids.py` when that file stopped carrying a
hand-bumped ceiling. It is the answer to "what was `EB-<n>`, and how did it
close" for every number the registers no longer show. It is history: nothing
here is maintained, and a new retirement appends its line to the end of the
first section rather than editing what is already written.

References to `OPEN_IDS` and to bumping `CEILINGS` are the mechanism as it stood
when each line was written; the mechanism now is `RETIRED`, above.

## Mint notes (from the `CEILINGS` block)
...
298 and 299, from the blind-render burn, are the two findings in the r4
Klee and r2 Kokomi records that no row carried. 298 is the MAP: the page
printed the adjacent nodes as bare labels while the feed was already
sending each option's `leads_to`, `nodes` for the whole act,
`current_position` and the boss's printed name -- "route choice is a
coin flip". 299 is TWO PRINTED LINES whose grammar a reader could not
...
EB-316/317 minted 2026-09-02 by the Kokomi round-3 packet (the
...
M62 minted 2026-08-29 by R221 A: the criterion that
...
Klee/Kokomi overhaul rounds, 2026-09-01/02
...
293/295: Kokomi round-two seat, text, HP
...
EB-749 RETIRED 2026-09-14: R271 stage one built (#481), through the doctrine door (`review/records/klee-stage-one-2026-09-14.md`) and read by round 27 (`review/records/klee-overhaul-round-27-2026-09-14.md`): the core decision holds in every seat, Grounded's condition is a fork, Return to Sender's cap read as printed.
EB-751 RETIRED 2026-09-14: the twelve custom resources name a shipped TexturePath (#485); a two-fight soak on the deployed build logged zero error lines and four seats played act 1 on it.
EB-756 RETIRED 2026-09-15: the act-4 scoping read is written (`review/records/teyvat-act4-scoping-2026-09-15.md`): a mod-added fourth act WORKS WITH A COST, three Harmony patches (two single-player) and a full asset set, the ending following the act count; the Abyss stays the act-4 face, waiting for MegaCrit stays the cheaper route.
EB-759 RETIRED 2026-09-15: TeyvatLoc merges from the LocManager.Initialize postfix (#496); the re-proof logged `teyvat: loc merged`, no NRE, and Nibbit read Wooden Shield Hilichurl Guard in a Mondstadt run.
EB-760 RETIRED 2026-09-15: the still scene is registered for BaseLib auto-conversion to NCreatureVisuals (#496); the Guard placeholder drew in two full Nibbit fights and neither ungated Spine timing door fired.
EB-761 RETIRED 2026-09-15: force_next_event (#497) opens the ? room on the named event with no rng consumed; the item-2 re-proof (`review/records/teyvat-spike-reproof-2-2026-09-15.md`) completed both options of the Springvale Cheese Cellar and observed The Chosen Cheese.
EB-764 RETIRED 2026-09-15: get_InitialPortraitPath falls back to the base event's image unless a dressed file exists (#500); zero AssetLoadException in the re-proof.
EB-765 RETIRED 2026-09-15: the dressed event's option rows are title/description pairs under TeyvatLoc.EventRows (#500); both options constructed, 8 distinct Commons and 2 taken on one, 14 unblockable HP and the relic on the other.
EB-758 RETIRED 2026-09-15: the no-track path asks DirAccess.DirExistsAbsolute, which does not log (#503); a three-fight soak with the arm on logged nothing from the music patches (`review/records/teyvat-fairness-2026-09-15.md`).
EB-762 RETIRED 2026-09-15: 24 embarks gave Mondstadt 10 / Liyue 14, 10 switches against 12.7 expected, discovery state identical before and after; the forced-undiscovered branch cannot fire for two IsDefault dressings. Fair; the earlier 8-then-6 ordering stays an unexplained oddity.
EB-763 RETIRED 2026-09-15: understudy prints the store's count and MB before launch and sizes the menu wait min(900, 180 + N/5) (#503); the stall itself is intermittent and is EB-766.
EB-757 RETIRED 2026-09-15: both atlases re-verified against the Fandom index, game8 and HoYoLAB (three corrections, four bodies added by name; statlines still unrecoverable) and scored as native columns (act-2 table §7, act-3 table §7): enemy-column-only, so neither outranks a ruled pair; default for a third face is Nod-Krai on Glory, Snezhnaya on the Hive.
EB-768 RETIRED 2026-09-15: the pck build's generated project.godot left convert_text_resources_to_binary at its default, so every layer scene shipped as a .tscn.remap stub that the layer-directory scan handed to the loader verbatim; #515 sets it false and adds a remap-stub probe that falls back to the base art. Installed 0.2.3314+proto arm ON: the pack carries the six Liyue layer scenes as plain .tscn entries and zero remap stubs, and the R225 soak passed (3 fights, 0 defects) with a complete dressed asset set present and no asset warning.
EB-741 RETIRED 2026-09-16: lint_unique_names.py calls console_safe() at the top of main(), pinned by a forced duplicate-name finding on a title carrying U+266A under PYTHONIOENCODING=cp1252 (#532).
EB-727 RETIRED 2026-09-16: FurinaResources.IsFurina takes a Creature? and reads creature?.Player?.Character, so null is false at the source and FurinaStage.LiveFor drops its local guard; KleeTests/FurinaIdentityNullTests pins both halves (#532).
EB-730 RETIRED 2026-09-16: test_records_stay_per_board_and_in_the_registered_order swapped its three sleeps for a threading.Barrier plus a wait on the pipeline's own read_order, with 30 s deadlock timeouts; ten runs of ten green beside two full fast lanes (#532).
EB-731 RETIRED 2026-09-16: KleeTests/StartingRelicPoolTests walks StartingRelics and each pool's GenerateAllRelics off the IL and compares the sets for every character and arm at once; seen to fail on the pre-#469 tree (#532).
EB-771 RETIRED 2026-09-16: the skip_act debug op landed in #534 and proofs-7 (#556) proved it -- act 2 reached 32.9-50.9 s from the start of Session.setup on six timed embarks, each skip itself 0.6-1.0 s, the last-act refusal firing by name.
EB-750 RETIRED 2026-09-16: the superseded KLEESPARK arm's rows and their generated C# are deleted, the SPARK_ALT_* maps dropped and codegen back in sync; no proto_spark_* id on the surface (#535).
EB-729 RETIRED 2026-09-16: the emitter holes the base literal as well as the upgraded one, so a whole-card conditional delta moves both printed numbers with no hand-authored hole (#538).
EB-712 RETIRED 2026-09-16: plan_carried_out_this_turn joins _ENGINE_LIVE_PREDICATES and a test enumerates every predicate the prototype surface prints, so no condition is priced at zero by omission (#539).
EB-661 RETIRED 2026-09-16: the blind-play combat-reward screen prints each item's type and rules text as the chest screen does (#536).
EB-708 RETIRED 2026-09-16: the enemy legend says (S)/(M)/(L) are sizes and Minion is a printed status (#536).
EB-701 RETIRED 2026-09-16: the battle page's turn-order note says end-of-turn effects resolve before the enemies act (#536).
EB-704 RETIRED 2026-09-16: EB-674's confirm note prints on every chooser path, the potion chooser included, with a fixture (#536).
EB-705 RETIRED 2026-09-16: the duplicate enemy list was a second append in the render, not the board; assert_one_page now covers the enemy list (#536).
EB-753 RETIRED 2026-09-16: the blind-play glossary is scoped to the arm the run's character owns, pinned on a fixture per kit, so no Stage or Kokomi keyword row prints on a Klee screen (#536).
EB-702 RETIRED 2026-09-16: skip on a reward screen with a card offer skips the offer, so the page's printed verb works (#546).
EB-700 RETIRED 2026-09-16: a folded face prints the base beside the current number, so a seat can tell a modified number from a base one (#546).
EB-707 RETIRED 2026-09-16: reachability reads the deck's and the powers' elements rather than the hand's, so the NO REACTION IS REACHABLE HERE line goes when a second element is in the run (#546).
EB-706 RETIRED 2026-09-16: the intent prints as the game will deal it, enemy Weak and player Vulnerable folded, said once (#546).
EB-715 RETIRED 2026-09-16: the act transition is printed with what it did to HP, gold and the deck, so no HP change goes unnamed (#546).
EB-662 RETIRED 2026-09-16: a potion buy on a full belt is refused with the belt count printed and the gold unspent (#546).
EB-409 RETIRED 2026-09-16: Strike Dummy was already fixed by EB-543 -- the relic reads the printed title, not a generated tag -- and #537 adds the pins on Slack Water and Deep Current that say so.
EB-766 RETIRED 2026-09-16: the boot-stall fuse is proven both ways -- proofs-6 (#540) met six stalls, caught all six at 48 s and relaunched each to a working menu, and proofs-7 (#556) then ran 39 consecutive launches with zero stalls, boot wall median 31.8 s and max 40.1 s, which is the row's own "30 consecutive launches with none over 90 s".
EB-748 RETIRED 2026-09-16: the unplayable_reason names the power or rule that refused the play; the fix was already on main and #541 adds the Smoggy pin.
EB-328 RETIRED 2026-09-16: HitOrder.BodyForPreview names the body once and hands it to the game's own var, so the two folding faces no longer multiply TargetMods on top of a hook that already folded them; 22 pin tests, 6 face-shape tests and 15 sim tests cover every modifier pair (#544).
EB-689 RETIRED 2026-09-16: made redundant by construction by EB-328's repair -- the target's terms are folded by the engine over one named body for every face -- pinned by Every_attack_face_takes_the_same_body_whatever_it_targets (#544).
EB-546 RETIRED 2026-09-16: cause found -- Sludge Spinner's opening OilSprayMove applies Weak 1, so the turn-two performance amplified from a Weakened tick (6 x 0.75 x 1.5 truncated to 6); already fixed by EB-588's unpowered performance, and #548 pins both arithmetics either side of it.
EB-466 RETIRED 2026-09-16: superseded -- R269 retired the Spotlight with the reframe arm, so the one-Companion designation has no system to be built into (#548).
EB-495 RETIRED 2026-09-16: the kit-verb / base-game-trigger matrix is written, 24 verbs by 8 triggers, 192 cells per engine and no cell unknown, with behavioural, AST and IL tests (#549); D1-D4 (#553) and D5/D6 (#557) repaired the one-sided sim parity defects it found. The matrix landed in docs/current/atlas/kit-verbs-vs-base-triggers.md rather than review/records, which is where the row's acceptance line said to put it.
EB-709 RETIRED 2026-09-16: Second Wave's tip says a Plan carried out twice counts twice for every per-Plan clause (#550).
EB-347 RETIRED 2026-09-16: the D default applied -- auto-plays exclude the Bake-Kurage, so an Uproar never writes a Plan, pinned in both engines (#550).
EB-331 RETIRED 2026-09-16: the D default applied -- the cadence exemption covers every off-sheet card, so a face printing no element applies none (#550).
EB-713 RETIRED 2026-09-16: the Hydro tag and the Applies Hydro glossary block print only on cards whose face-up hit applies the element (#550).
EB-613 RETIRED 2026-09-16: a Geo card's preview names the consumption on the page and in KleeCardTooltips (#550).
EB-614 RETIRED 2026-09-16: a no-damage applier over an amplifiable aura previews the strip with nothing to amplify (#550).
EB-615 RETIRED 2026-09-16: render_card_gallery.py's scraper reads the FurinaBurstRider.Face wrapper, so a rider-wrapped row's tile shows its in-game text (#551).
EB-387 RETIRED 2026-09-16: NOT the reaction table -- both sightings are one turn after RAZOR, so Lightning Fang made the hit Electro into an Electro aura, a refresh that fires nothing; ReactionTable is unmoved and EB-389 is the repair (#554).
EB-561 RETIRED 2026-09-16: the face half landed as EB-713 (the generic Applies Hydro keyword came off these rows, leaving the rider as the one aura statement, the row's acceptance word for word); #554 pins the hit against that statement.
EB-562 RETIRED 2026-09-16: the Casket's strike does leave a Hydro aura, always did, and the reaction glossary's sources clause now admits the exception and points at the relic's own keyword line (#554).
EB-714 RETIRED 2026-09-16: the row misread its own seat -- the body was at 11, the first Plan took it to 3 and it was alive when the second arrived, so the wasted 5 was overkill, not a corpse; the aim is re-read per entry and filtered to the living, and #554 pays the gate the row owed in both engines.
EB-726 RETIRED 2026-09-16: the FURINA_REFRAME arm is out of the tree whole (#561) -- 224 files, the sim module, six C# production files, both build switches, ~30 engine seams, 22 test modules, the codegen emitter and the page's salon block; the six reframe faces are frozen in docs/notes/retired-prototype-rows.yaml so the sealed rounds 1-16 stay readable, and STATE names one Furina arm, furina-stage.
