...
Seven number shaves (Workshop/Spark-Knight-Style/Grand-Finale scaling −1 each; four spark block riders −1 block each). Bursts gain Retain per principles v1.4. Rationale + empirical elasticity in code-review round 2 notes. Pass-1 also shipped stack caps on the scaling powers; the 2026-07-22 playtest rebalance (`b5bb92e`) removed them, and [USER] reaffirmed uncapped 2026-08-06 (`X8-cap`, ruled and retired with `ebcf98e`) — duplicate Klee powers stack without caps, pinned in `test_pass2.py`.
