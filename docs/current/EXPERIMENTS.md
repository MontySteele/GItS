...
- **`KLEESPARK-W5` — the whole fight R230 left owed (`EB-224`)** — **DRAFTED 2026-08-30, not countersigned. Its sibling `KLEESPARK-BT3` RAN 2026-08-30 and is in the Graded table above**, so what stays active here is the whole fight alone. `BT3` did not block it: §25.3's gate blocks `W5` only on a `G1` MISS at engine clause (ii) or a `G3` MISS, and `G1` came back UNREACHED while `G3` PREDICTED. Drafted by
