# kokomi-cards.yaml - comment provenance

Long comment blocks that used to sit in `docs/kokomi-cards.yaml`. They
...
# R80 (STANDING LAW, healing-law register — Neap Tide v2.1), AMENDED BY R226 (2026-08-30):
#   CHARGE IS SPENT BY THE BAKE-KURAGE AND BY NOTHING ELSE. Uncapped, accrued at 1 per Exhaust of one of her
#   own cards (Companions included, Status and Curse excluded), card-event-driven with no passive accrual.
#   ONE DOOR ONLY: at the threshold the jellyfish pays the bank to play the front of its memory for 0 energy,
#   one card per turn. THE ANTI-CONVERGENCE CLAUSE STANDS AND IS THE POINT: no card prints a Charge cost and
#   no card reads the bank proportionally, so no Encore-shaped face can enter her pool by precedent. R80's own
#   warning is why every number below is marked re-derive rather than kept — "a bank that can be spent is a
#   different resource wearing the same name, and every per-point read here would be describing a curve that
#   no longer exists" — and the firewall it built against Regent-Stars convergence moves from "never spent" to
#   "spent one way, by the kit, on tempo and never on magnitude."
#   *** PROSPECTIVE (R213), countersigned R226: this block binds when C.KURAGE_MEMORY flips. Until the flip
#   the SHIPPED rule is the one it replaces — CHARGE IS NEVER SPENT, read or thresholded, never expended —
#   and that is still true on both sides by construction (ChargeResource.Spend is a documented no-op and tier0
#   has no spend_charge). No row on this sheet moves on this comment. ***
...
#   review/records/payoff-reach-reregistration.md §6.8. Phase-0 item 8 forbids the alternative in so many words
