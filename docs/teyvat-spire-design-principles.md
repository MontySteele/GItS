...
- **Signature companions:** 1–2 lore-linked 4-stars live in a character's *personal* pool instead (Klee → Albedo). Chosen for story resonance AND for patching the character's statline weakness.

### 4.2 The enabler-not-carry principle (load-bearing)
Companion cards apply elements, generate small utility, and feed Burst Energy. They are **never** the deck's damage engine, never scale, and never exceed uncommon-grade power. Test: if deleting your character's own cards from a winning deck wouldn't gut it, companions are too strong. Genshin analogy holds: your 5-star is the carry; 4-stars enable.

### 4.3 v2 candidate — the Wish banner
...
3. Companion cards: ≤ uncommon power grade, no scaling effects, no archetype-payoff effects.
