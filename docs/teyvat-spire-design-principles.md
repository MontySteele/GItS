...
- **Energy generation** ⚠️ the one real change. No neutral or companion energy source survives — companions grant `burst_energy` (the meter), never real energy. **Ruling: raw energy generation is character-kit-only, by design** — it keeps energy economies character-defined and leaves room for a future "battery" character, and companions sourcing raw energy would be a §4.3 breach on the most premium resource. *Obligation:* every character's pool must self-provide enough energy/tempo, since colorless energy-splashing is gone. Klee currently ships exactly one (`sugar_rush`) — **flag to validate sufficiency in Tier 0 before lock**; Furina has three, fine.
