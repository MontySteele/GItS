...
**Status 2026-07-23 — the 6 identity assets are DONE.** `select_portrait`
(+ auto-derived `select_portrait_locked`), `char_icon`, `map_marker`, and
`combat_model` are sourced from the clean transparent `Furina Profile` render;
`selection_splash` from the `Furina Wish` full composition. Outputs live under
`ImageGen/images/furina/{ui,model}/`, so `build_pck.ps1` no longer falls back
