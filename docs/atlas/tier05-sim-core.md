...
with no combat state; `cells.py` is the identity object that makes two run
numbers comparable (R68). Scope boundary, verbatim: *simulate what WE design
(cards, pools, reward slots, draft flow); measure what MegaCrit designed with
Tier 1/2 on the real game* (`__init__.py:4-5`). It is explicitly **not** the
combat engine (`tier0/engine/` is emit-only; its battery is frozen and no longer
read here — `acts.py:26-29`, `model.py:86-89`), **not** an authored spine any
more (the 11-node template died at RUNTEMPLATE 6 — `model.py:236-241`), and it
must not be re-implemented inside `understudy/`, which drives the real game and
"is not a simulator and it must never become one" (`understudy/README.md:10`).
...
Experiment scripts go through a `Cell`, which carries the stamp a report needs
to be citable:
...
- **One `random.Random` per run**; fight seeds, reward rolls and map rolls all
  flow through it (`model.py:19-21`, `acts.py:21-24`, `maps.py:22-23`). Side
  measurements take DEDICATED streams so they cannot renumber a run: banner
  `seed+2e9` (`model.py:284-295`), starting deck `seed+3e9` (`:298-300`),
  draft_regret `seed+i+1e9` (`:779-781`).
...
- **Reward screens: the fight's own, then any it EARNED.** An extra screen
  ...
  never the forced-Rare tier (`model.py:684-685`, `:718-723`). A non-final boss
  forces Rare on BOTH card offers and companion slot (companion-only was the
  Ironclad-0.6% bug), then heals to full (`:662-670`, `:733-744`).
- **Pool YAML has a closed key schema**: `_validate_pool` raises on any unknown
  tier/encounter/enemy/intent key, because pools are read via `.get()` and a
  typo (`is_bos: true`) is otherwise a silent no-op (`acts.py:53-103`). Cached
  by FILENAME so a monkeypatched `RUN_ACTS` cannot serve a stale entry, and read
...
- **The boss draw always consumes an rng call**, even from a 1-entry pool, so
  growing a pool never renumbers the other streams (`acts.py:211-212`).
...
- **`STATIC_OP_PRICING` must cover the engine's whole `OPS` registry**, priced
  ...
  the build otherwise (`draft.py:986-1003`).
- **`draft.py` defines no version constant** — the live stamp is
  `DRAFTER_VERSION` in `tier0/constants.py` (`draft.py:9-11`), and a scorer
  change IS a version bump in the same edit (`tier0/DECISIONS.md:2658`).

## 4. Rulings that shaped it

- **R2.1 / R2.2** — the hybrid's power term was adopted into `score_offer`
  (`hybrid_policy` is now an alias) and reaction got a steeper lean line (cap 13
  × 0.4); raising applier/amp measured worse
...
- **R61** — the sim MODELS the shop as an economy channel; hence the `$` branch,
  and offers logged separately from purchases (`tier0/DECISIONS.md:1813-1824`;
  `model.py:392-453`, `:169-174`).
- **R64** — the Featured Banner went live and `roll_banner`'s `nations` must be
  passed explicitly from the sheets; the argument-free default had filtered
  every non-Mondstadt 5-star out of every run
...
- **R66** — archetype vocabulary is the sheet's; adaptive numbers taken through
  ...
  through `runner.py`'s registry (`tier0/DECISIONS.md:1989-2060`;
  `draft.py:1284-1314`).
- **R68** — the canonical cell: `Cell` is the single source of truth for cell
  identity, versions are read live and never stored, plan→pilot resolves only
  through `runner.resolve_plan`, and **a report without a stamp is not citable**
  (`tier0/DECISIONS.md:2122-2163`; `cells.py:61-149`, `runner.py:62`).
- **R83** — the draft-scorer pass is the authorized policy work; anchor role
  labels do not track generic value, so plan bonuses scale on
  `archetype == "generic"` only (`tier0/DECISIONS.md:2623-2656`;
  `draft.py:689-736`, `:1154`).
- **R84** — DRAFTER 11 ratified; `STATIC_DEXTERITY_VALUE` added as the Strength
  mirror, `STATIC_POWER_ENGINE_VALUE` held at 0.0 as a *documented dead dial*
  (`tier0/DECISIONS.md:2658-2698`; `draft.py:738-773`).
...
- **Every published number is world-stamped, and worlds are not comparable.**
  ...
  `:821-917`). Check the stamp before quoting anything.
- **`node_template()`, `ELITES_PER_ACT` and `RUN_NODE_TEMPLATE` are DEAD but
  must not be deleted** — they name the v5 spine archived tools were measured on
...
- **`route_regret` does not exist.** The research doc mandates it as a day-one
  countermeasure and `route.py`'s header once claimed it shipped; nothing
  defines it (`route.py:13-15`, `docs/missed-requirements.md:93-104`).
  hunter-vs-cautious is the only real route countermeasure.
- **Do NOT calibrate `MAP_ROOM_ODDS` or the route policies against a winrate.**
  ...
  tuning a difficulty dial to a winrate already had to be retracted once here
  (`route.py:17-27`, `tier0/constants.py:656-660`,
  `tier05/tests/test_maps_and_routing.py:1-10`).
- **The map generator carves paths; it does not roll widths.** The first version
  rolled per-floor widths and produced lane-locked maps where 9% of
  elite-hunting routes reached zero elites — widening floors did not help,
  because *connectivity* binds (`maps.py:91-111`). Elite-not-twice-in-a-row is
  enforced at walk time by the draw, not the generator (`maps.py:12-14`,
  `acts.py:226-233`), and `resolve_unknown` MUTATES the weights dict it is
  handed — the real pity rule, reset per act (`maps.py:155-182`).
- **Priced-at-zero drafter constants are measurements, not oversights**
  (draw/energy/spark/burst, `raise_fanfare_cap`, `crash_fanfare`, `strip_block`,
  `GENERIC_REDUNDANCY_PENALTY`, `STATIC_POWER_ENGINE_VALUE`); each is a *named*
  dial so the next pass starts from the measurement
  (`draft.py:797-813`, `:721-725`, `:754-773`).
- **Two latent scorer footguns**: `reaction`'s lean penalty STACKS with the
  global bloat line past `DRAFT_DECK_SOFT_CAP` (unreachable today — re-sweep
  before trusting either coefficient past 22 cards, `draft.py:1219-1226`), and
  `_drafted_readers` excludes basics on purpose, because Furina's starter
  carries a Fanfare read that would otherwise close the reader limb at run start
  forever (`draft.py:244-261`).
- **`cells` ↔ `runner` is a deliberate one-way module edge**: `cells` imports
  `runner` at *call* time, `runner` imports `cells` inside `main()`; moving
  either to module scope reintroduces the cycle (`runner.py:25-30`,
  `cells.py:49-58`). `--ab` is refused for Furina (`runner.py:126-131`), and the
  CLI forces stdout to UTF-8 because the death-heatmap glyph killed the report
...
3. `tier05/maps.py` then `tier05/route.py` — how node kinds and the path are
   produced, and the confounder warning governing both.
4. `tier05/acts.py` — encounter pools, the key allowlist, `ActDraw`.
5. `tier05/draft.py` — module docstring, `core_complete`/`_core_progress`,
   `score_offer`, `assigned_policy`, then the constants blocks (which are the
   measurement record).
