...
# EB-249 (a): the companion sheets are GLOBBED, not listed. The list named
# `mondstadt-companions.yaml` and nothing else, so Fontaine's and Inazuma's
# sheets were absent from the input entirely -- a different thing from the
# size exemption at GATE_MIN_POOL, which measures a pool, prints it and names
# it as skipped. Absence reads as coverage. A glob is what makes the fourth
# nation's sheet arrive on its own.
...
# row sit in a sheet without being in that pool:
#
#   `personal_pool`  a Personal Companion, draftable only inside one
#                    character's own pool (Prune is Klee's). A real card, and
#                    not in the shared pool the ratio is about.
#   `guest_star`     a temporary cameo generated by a Furina mechanism
#                    (`generate_guest_star`), never drafted by anybody.
#
# This is the predicate the loader already applies at
# `guest_star_generation_pool` and `companion_pool`, STATED here rather than
# inherited, because this tool reads the sheets and not the loader's index.
# Field-based, so it is a no-op on every sheet carrying neither -- all three
# character sheets today, and the point is that it stays a no-op there.
TAXONOMY_KEYS = ("personal_pool", "guest_star")


def universal_rows(rows: list[dict]) -> list[dict]:
    """The draftable Universal rows of a sheet: everything the taxonomy does
    not file somewhere else."""
    return [c for c in rows
            if not (isinstance(c, dict)
                    and any(c.get(k) for k in TAXONOMY_KEYS))]
...
def build_reports(pool_filter: str | None = None,
    ...
    for path in SHEETS + GAME_REF:
        ...
        # EB-249 (b). AFTER the read and before any measurement, so an
        # unparseable sheet is still the hard failure above and the filter
        # only ever narrows a pool that was fully read.
