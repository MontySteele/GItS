...
MECHANICAL_OPS = {"damage", "block", "draw", "place_bomb", "gain_spark",
                  ...
                  "split_largest_bomb",
                  # POOL PASS TWO's two (`EB-730`), and neither touches a Bomb.
...
PREDICATES_CS = {
    ...
    # would be a row printing a rule this kit does not have.
...
#: POOL PASS TWO's two (`EB-730`), same discipline and both bare: neither
...
APPLY_POWERS = {
    ...
    "ko_vermillion_pact": ("VermillionPactPower", None,
        ...
        "triggers one too."),
    # POOL PASS TWO's two (`EB-730`). Return to Sender is a MARK on the Block
...
def card_level_reason(
    ...
    # `EB-730` (Blast Shield). `return_to_hand` IS A FACT ABOUT THE WHOLE PLAY
...
def blocked_reason(
    ...
    for eff in card.get("effects", []):
        ...
        # POOL PASS TWO's two (`EB-730`), same UNPARSEABLE discipline.
...
def stage_count_block_rider(card: dict,
    ...
    `fanfare_drained_block_rider` opened this rail with.
    """
    if eff.get("op") != "block":
        return None
    formula = eff.get("amount_formula")
...
def block_calc_rider(card: dict, eff: dict) -> tuple[int, int, str] | None:
    ...
    *block* op and the generator dropped it without a word: the sheet said
    ...
    if (not m and not members and not companions and not charge
            and drained is None and staged is None):
        return None
    if salon_deploy_card(card):
        return None
    effects = card.get("effects", [])
...
def build_body(
    ...
    for eff in card["effects"]:
        ...
        # no method-scope local is declared.
            ...
            # BLAST SHIELD (`EB-730`), and it emits NO STATEMENT here on
            ...
            # ONCE MORE! (`EB-730`). ONE call into the ledger, which is where
...
'''
    ...
    # `EB-730` (Blast Shield): "Return this card to your hand."
    #
    ...
    return_to_hand_member = (
        "\n\n    // `EB-730`, Blast Shield: the played card goes back to the"
        " HAND\n"
