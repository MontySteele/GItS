...
# EB-315. Which `plan:` op each `plan_*` delta key binds to. Imported for the
# same reason BURST_PER_SKILL_TAG above is: the sim's applier binds these keys
...
MECHANICAL_OPS = {"damage", "block", "draw", "place_bomb", "gain_spark",
                  ...
                  # (`drain_fanfare`, the retired reframe's drain, left with
                  # its C# half, `FurinaDrain`, under R276's hygiene: no row
                  # printed it, and a row that tried now fails as an unknown
                  # op rather than emitting a call to a deleted class.)
                  ...
                  "grow_largest_bomb",
                  # THE POOL PASS's two (`EB-491`), on the same terms as the
                  ...
                  # and is Compile Remove'd out of a release build, so the only
                  # rows that may print these are `proto_` rows on the
                  # prototype surface, compiled under the same switch.
                  # Every one has a verified call site on `KokomiRules`,
                  # `KokomiPlan` or `KokomiOverhaulKit`; the whitelist stays
                  # honest.
                  #
                  # DRAFT 6 RETIRED SEVEN OF THE TEN. `gain_tide`, `surge`,
                  # `block_half_surge`, `exert`, `draw_per_tide`,
                  # `play_top_of_draw` and `draw_companion_from_draw` printed
                  # the Tide, the Surge and the Exert, which the ruled brief's
                  # sec.6 cuts by name ("Cut and not coming back: Tide, Surge,
                  # Exert, the pulse, Orders, Tactics, Spent, Garment as a
                  # keyword"). They are gone from here rather than left inert,
                  # so no row can print a rule the arm no longer has.
                  #
                  # `plan` IS NO LONGER AN OP EITHER. Draft 6's Plan is the
                  # second HALF of a printed face -- what the card does if it
...
# `KokomiRules.Mend` and printed nowhere.
#
...
ARM_KEYWORDS = (
    ...
    # not there"), and `tier0/tests/test_arm_keyword_tips.py` enforces it from
    # the other side -- every word here must be printed by some prototype face.
    ...
    # and not a bar, a shipped Salon member has no bar to raise or spend, and
    # nothing shipped bows for a spend or rotates without a deploy -- so all
    # seven are arm words and none is a shipped keyword.
    #
    # TWO OF THEM ARE TWO WORDS, `lead performer` and `back performer`, and
    # that is deliberate: the rules they carry are about WHICH SEAT, and a tip
...
PREDICATES_CS = {
    ...
    # each add one. A LIVE read, not a snapshot like the two aura predicates
    # above: this card applies nothing before its own branch, and what it asks
...
#: THE POOL PASS's two (`EB-491`), same discipline.
...
#: exactly these clauses and a clause outside this table is a build failure
#: rather than an approximation -- the same UNPARSEABLE discipline the field
#: whitelists above keep, one level down. `apply_power` resolves through
#: `PLAN_APPLY_POWERS` instead, because Weak and Vulnerable are two kinds.
PLAN_CLAUSE_KINDS = {
    "draw": "Draw",
    "energy": "Energy",
    "block": "Block",
...
#: buff onto an enemy by a typo.
PLAN_APPLY_POWERS = {"weak": "ApplyWeak", "vulnerable": "ApplyVulnerable"}

#: Where a plan clause lands. Rule 3: "A planned hit lands on the front enemy
#: (leftmost alive) unless the line says every enemy." The self-facing clauses
#: below take `Aim.Self` and print no target at all.
PLAN_AIM_CS = {
...
def plan_reason(card: dict) -> str | None:
    """Why this row's `plan:` list cannot be emitted, or None.

    The Plan is the second HALF of a printed face -- what the card does when
    it is played on the Bake-Kurage instead of where it would normally go --
    so it is checked exactly as hard as the body: closed clause table, closed
    target spellings, literal positive amounts.
    """
    body = card.get("plan")
    if body is None:
        return None
    ...
    for eff in body:
        ...
        op = eff.get("op")
        if op not in PLAN_CLAUSE_KINDS:
...
#: `EB-315`. The DynamicVar each `plan_*` delta declares, beside the ops it
#: binds to (`upgrades.PLAN_DELTA_OPS`, imported rather than restated). Every
#: name is prefixed `Plan` so a two-line row's two halves cannot collide in
#: `DynamicVarSet`, which throws on a duplicate name at reward time.
#: `PlanCards` rather than `PlanDraw` for the same reason the base game names
#: its draw var `Cards`: the token also drives `{...:plural:|s}`.
PLAN_UPGRADE_VARS = {
    "plan_damage": "PlanDamage",
...
def plan_var_effects(card: dict) -> dict[int, tuple[str, str]]:
    """`{index in `plan:`: (delta key, var name)}` for the clauses that upgrade.

    `EB-315`. A Plan clause is emitted as a literal amount unless this card's
    upgrade MOVES it -- the Sparks idiom the whole file keeps, so a row with no
    plan delta emits exactly the `KokomiPlan.Planned` record it always did and
    no generated file churns for a card whose upgrade never touches it.

    The FIRST clause of each key's op owns the var, which is the same clause
    `upgrades.apply_upgrade` bumps (its `PLAN_DELTA_OPS` walk). Both sides read
    one table, so the two engines cannot upgrade different clauses of one Plan.
    """
...
def plan_clause_cs(eff: dict, var: str | None = None,
    ...
    moves it (`plan_var_effects`). `PlanClauses` is a PROPERTY on the card
    instance, so reading the var there is what makes `KokomiPlan.ResolveAll`
    carry out the upgraded number: the clause is built fresh at enqueue time
    off the smithed card rather than baked into a static literal.
    """
    op = eff["op"]
...
APPLY_POWERS = {
    ...
    #
    # DRAFT 2's FIVE ARE GONE with the rules they printed: the Garment, the
...
EXPRESSIBLE_DELTAS = ({"damage", "block", "draw", "spark",
                      ...
                      # DynamicVar the emitted `PlanClauses` reads back, which
                      # is what makes `KokomiPlan.ResolveAll` carry out the
                      # UPGRADED value and the `+` face print it green.
                      # Prefixed rather than sharing the now-line's keys so a
                      # two-line row moves both halves without either applier
                      # guessing which line a bare `damage` meant.
                      | set(PLAN_DELTA_OPS)
                      | POWER_UPGRADE_KEYS)
...
CARD_FIELDS = {
    ...
    # `KokomiPlan.Planned` clauses -- never as a captured closure over a
...
def cs_escape(text: str) -> str:
    ...
    escape this adds would be escaped a second time. A line break becomes
    the two characters backslash-n: the base game's own reader faces put
    their live number on a line of its own (`{InCombat:<break>(Deals ...)|}`),
    and a raw break inside a C# string literal does not compile."""
...
def blocked_reason(
    ...
    for eff in card.get("effects", []):
        ...
        if op == "damage_quarter_max_hp":
            unknown = set(eff) - DAMAGE_QUARTER_MAX_HP_FIELDS
            if unknown:
                return (f"damage_quarter_max_hp field(s) {sorted(unknown)} "
                        "not understood")
            if eff.get("target") not in {"enemy", "all_enemies"}:
                # The now-line aims where the player pointed; the PLAN half of
                # the same row lands on the front enemy and is checked by
...
def stage_count_calc_rider(card: dict,
    ...
    read, so it is refused unless the card actually spends FIRST.
    """
    if eff.get("op") != "damage" or eff.get("target") == "self":
        return None
    formula = eff.get("amount_formula")
...
def stage_count_block_rider(card: dict,
                            eff: dict) -> tuple[int, int, str] | None:
    """The same counts on a BLOCK op -- *Pneuma Refrain* and *Final Bow*.

    `base: 0` is legal and is what both rows print: "Block equal to X" is a
    pure slope with no printed floor, which is the shape
    the retired reframe's drain rider opened this rail with.
    """
    if eff.get("op") != "block":
        return None
    formula = eff.get("amount_formula")
...
def upgrade_plan(card: dict) -> tuple[dict, str | None]:
    ...
    has = {
        ...
        # `EB-491`. Split Charge's upgrade-only number. It binds to the op
        # that prints it, the one-owner rule every key here keeps, and is
        ...
        "formula_per": any(
            ...
            # reads. The BLOCK rail is here and not on the damage list above
            # it because `Pneuma Refrain` and `Final Bow` are its first rows:
            # both print "Block equal to X", a pure slope whose upgrade is a
            # printed floor.
            or stage_count_calc_rider(card, e) is not None
            or stage_count_block_rider(card, e) is not None
            for e in effects),
        "formula_base": any(
            exhaust_pile_calc_rider(card, e) is not None
            or exhaust_selection_calc_rider(card, e) is not None
    ...
    # EB-315. A `plan_*` delta needs a CLAUSE OF ITS OP carrying a literal
    # amount on the row's `plan:` line -- the same requirement every key above
    # states about `effects:`, one printed line over. A plan delta on a row
    # with no Plan (or with no clause of that op) is a sheet/card mismatch and
    # is reported as one rather than silently dropped.
    for plan_key, plan_ops in PLAN_DELTA_OPS.items():
        has[plan_key] = any(e.get("op") in plan_ops
                            and isinstance(e.get("amount"), int)
                            for e in plan_line)
...
def _emit_branch_op(
    ...
    if op == "damage":
        ...
        # `build_body`, and an arm missing from this resolver is a card that
...
#: rather than a meter price: `{op: (C# predicate, the refusal's words)}`,
...
def build_body(
    ...
    for eff in card["effects"]:
        ...
        # is inside `KokomiRules.Mend` and NOT at any call site, which is what
        # makes "no Mend goes above entry HP" a property of the code.
        #
        # THE PLAN HALF IS NOT HERE. A row's `plan:` list is emitted as the
        # typed `PlanClauses` member and the play-on-the-jellyfish branch at
        # the top of `OnPlay` (see `emit`); these arms are the NOW-lines.
        elif op == "mend":
            lines.append(
                "await KokomiRules.Mend(choiceContext, Owner.Creature, "
            ...
            # Sango Isshin's now-line. The quarter is computed in ONE place
            # (`KokomiRules.QuarterMaxHp`) so the printed face, the aimed hit
            # and the planned all-enemies version cannot round differently.
            if eff["target"] == "enemy":
                _target_guard(lines, ctx)
                lines.append(
            ...
            lines.append(
                "await KokomiOverhaulKit.RemoveOneDebuff("
                "choiceContext, Owner.Creature);")

        elif op == "carry_out_front_plan":
            # Change of Plans. The jellyfish carries out the FRONT Plan now and
            # it leaves the queue, which is what "carries out" means everywhere
            # else in the arm -- one resolution, not a copy.
            lines.append(
            ...
            # play that chose it is long gone.
            lines.append(
...
def _authored_face_numbers(card: dict):
    ...
    # clause the upgrade does not move still yields, for the same reason every
    ...
    for index, eff in enumerate(card.get("plan") or []):
        ...
        if not isinstance(amount, int):
            continue
        if index in plan_vars:
            key, var = plan_vars[index]
...
# pluralize the noun beside it: "draw 1 card" upgrades to "draw 2 cards", and a
# token that swapped only the numeral would print "draw 2 card". The rendered
# path has always emitted `{Cards:plural:|s}` beside its draw token; this is
# the same pair for a row that states its own face.
CARD_COUNT_FACE_VARS = frozenset({"Cards", "PlanCards"})
...
def _authored_face_with_tokens(card: dict) -> str:
    ...
    for key, var, literal in _authored_face_numbers(card):
        # `EB-315`. A card COUNT takes its noun with it, so the swap can put
        # the plural token where the noun was -- "draw 1 card" has to become
        # "draw 2 cards" and not "draw 2 card". The noun is optional in the
        # pattern so a count printed without one still matches.
...
def build_upgrade(card: dict) -> list[str]:
    ...
    # EB-315. The PLAN line's own numbers, in the order the row wrote its
    # clauses so the emitted OnUpgrade reads down the face. Each moves the
    # per-clause var the emitted `PlanClauses` reads back, which is the whole
...
def emit(
    ...
    # spellings it takes is decided by what the card does when it is NOT
    # planned:
    #
    #   * no now-line at all      -> the pet alone (`CustomTargetType.Pet`);
    #   * a now-line that aims    -> an enemy or the pet;
    #   * anything else           -> you or the pet (`PetOrSelf`).
    #
    # The base library ships the predicates, the validation and the
    # drag-to-target patches for `Pet` and `PetOrSelf`; the middle one is the
    # arm's own custom type, registered on the same seam (`KokomiTargets`).
    plan_body = card.get("plan")
    if plan_body:
    ...
    if plan_body:
        ...
        # was targeted (`CardPlay.Target`), so "was this played on the
    ...
    # off a card it was handed, which is what Moon's Reflection needs: it
    # replays a chosen card's Plan LINE, and a private member could not be
    # asked.
    #
    # EB-315: A CLAUSE THE UPGRADE MOVES READS ITS AMOUNT OFF A DynamicVar,
    # and it works because this is a PROPERTY rather than a field -- the record
    # is built at enqueue time off THIS instance, so a smithed card writes the
    # upgraded number into the queue and `ResolveAll` carries out that one. A
    # clause no delta touches keeps its literal (the Sparks idiom), so no
    # generated file churns for an upgrade that does not reach it.
    plan_member = ""
    if plan_body:
        plan_vars = plan_var_effects(card)
        alt_vars = plan_alt_var_effects(card)
        ...
        plan_member = (
            ...
            "Bake-Kurage at the start of her next\n    /// turn "
            "(<see cref=\"KokomiPlan.ResolveAll\"/>).</summary>\n"
    ...
    if profile.arm_keyword_tips:
        ...
        if merges_bombs(card):
            tips_expr = (
                ...
                f"{tips_expr or 'base.ExtraHoverTips'}, this)")
        # (The Stage readers' "What this number is" rider left on
        # 2026-09-25: Let the People Rejoice, its last row, deals twice its
        # Fanfare and says so on its face.)
