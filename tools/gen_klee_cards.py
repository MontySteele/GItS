...
MECHANICAL_OPS = {"damage", "block", "draw", "place_bomb", "gain_spark",
                  ...
                  "split_largest_bomb",
                  # POOL PASS TWO's two (`EB-724`), and neither touches a Bomb.
...
#: POOL PASS TWO's two (`EB-724`), same discipline and both bare: neither
...
APPLY_POWERS = {
    ...
    "ko_vermillion_pact": ("VermillionPactPower", None,
        ...
        "triggers one too."),
    # POOL PASS TWO's two (`EB-724`). Return to Sender is a MARK on the Block
...
def card_level_reason(
    ...
    # `EB-724` (Blast Shield). `return_to_hand` IS A FACT ABOUT THE WHOLE PLAY
...
def blocked_reason(
    ...
    for eff in card.get("effects", []):
        ...
        # POOL PASS TWO's two (`EB-724`), same UNPARSEABLE discipline.
...
def build_body(
    ...
    for eff in card["effects"]:
        ...
        # no method-scope local is declared.
            ...
            # BLAST SHIELD (`EB-724`), and it emits NO STATEMENT here on
            ...
            # ONCE MORE! (`EB-724`). ONE call into the ledger, which is where
...
'''
    ...
    # `EB-724` (Blast Shield): "Return this card to your hand."
    #
    ...
    return_to_hand_member = (
        "\n\n    // `EB-724`, Blast Shield: the played card goes back to the"
        " HAND\n"
