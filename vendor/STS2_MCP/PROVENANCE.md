...
| `gits/GitsLocalEvent.cs` | GItS addition | Live look 8b (2026-09-16) — one guarded read, so a state call on the Event Room path cannot die on the game's own indexing. `get_state` answered twice with `ArgumentOutOfRangeException at EventSynchronizer.GetEventForPlayer at McpMod.BuildEventState`, losing the WHOLE screen rather than the options. The call site is ours: `EB-682`'s fallback reads `eventRoom.LocalMutableEvent`, which is `RunManager.Instance.EventSynchronizer.GetLocalEvent()`, whose body (decompiled, sts2.dll 0.111.0 `41cef1ea`) is an unguarded `_events[_playerCollection.GetPlayerSlotIndex(player)]` — a slot the collection cannot name answers -1, and an `_events` array not yet sized for the room is shorter than the index. The game never meets it because its own callers are inside a begun event; a poller meets the moment before. Falls back to the room's CANONICAL event, which is what every other field on that screen is read off, so the answer is the empty-options shape at worst. Logged once per process (a poller would print a line a frame). Names no game type — the read is handed in as a callback from `McpMod.StateBuilder.cs`, `GitsSettledHp.cs`'s rule. Read-only. |
| `gits/GitsSettledHp.cs` | GItS addition | `EB-676` (2026-09-16) — whether the HP figure on a state payload has SETTLED, and nothing else. There is exactly ONE HP field on this wire (`BuildPlayerState` writes `creature.CurrentHp` on every screen), so the Kokomi r26 victory screen's 25/80 and the next screen's 16/80 were that field at two moments, the first taken before the fight's own end-of-turn queue (Constrict's 9) had drained into it, and the seat chose its map off the wrong number. The predicate has two clauses and BOTH are needed: (1) nothing is executing — `RunManager.Instance.ActionExecutor.IsRunning` is false and its `CurrentlyRunningAction` is null, an HP write one action away being the whole of the defect; and (2) no combat is standing in its own teardown — `CombatManager.Instance.CurrentCombatId` is null OR `IsOverOrEnding` is false, which is to say a combat object still exists (`Reset` drops `_turnState` wholesale when the run leaves the room) AND it has ended. **`!IsInProgress` is NOT the predicate** and is the obvious wrong answer: the game's own summary of that property says it is "False when ... the combat is ending (the last monster has been killed)", so a flag built on it would have read `settled` on the exact frame the defect was filed against. A live fight sitting in the play phase with an idle queue IS settled, deliberately — a flag that cried provisional through every combat page would be a flag nobody reads. Free of Godot, Harmony and game types on `GitsPort.cs`'s and `GitsSkipAct.cs`'s terms and for their reason; `klee-mod/KleeTests/GitsSettledHpTests.cs` compiles THIS file, not a fork. Fails closed: the four live facts are read inside a try/catch in `McpMod.StateBuilder.cs` and any throw publishes `hp_settled: false` with a reason naming the throw. Read-only. |
| `gits/GitsPileCard.cs` | GItS addition | `EB-789` — one row builder for every card pile, and the whole of it is "ask the hand's question". `BuildPileCardList` is called for `draw_pile`, `discard_pile`, `exhaust_pile` and (since `EB-447`) `master_deck`, and it built a lightweight four-key row beside `BuildCardInfo` rather than through it — so a pile face and a hand face were two lists of keys kept in step by hand, and they had drifted in both the ways that matter: no `enchantment` block (`EB-181` added it to `BuildCardInfo` only) and a `description` taken at the card's OWN pile, which is the face that does not fold the enchantment's number. Hence *Gain 5 Block* in the draw pile and *Gain 7* in hand one turn later. `description` is the HAND face deliberately — the face the card will present when it is played is the only one a reader planning with it can use — and the pile-CONTEXTUAL sentence is kept as `pile_description`, emitted ONLY where the two differ, so an absent key states "this card reads the same wherever it is". `pile` names the pile the row came out of, because the four lists reach the page through one reader. Read-only; names no private member and takes no reflection. |
| `gits/GitsCardSelection.cs` | GItS addition | `EB-263` — which card a selection screen is holding, which nothing on the wire said: `BuildCardInfo` has no selection field and the only thing that moved when a pick landed was `can_confirm`, so a blind tester who chose a card got the same page back. Read off `NCardGrid._highlightedCards` by reflection — the grid's own paint list, which is what the screen shows and hides each holder's highlight from, so the flag on the wire and the glow on the card cannot disagree. Null on any failure and never false, with `selection_known` carrying that difference to the page. **`EB-794` (2026-09-16) — and one screen arms its pick somewhere else.** The shop's removal grid marked no row at all after a `select_card` that armed a correct pick and a confirm that took it (proofs-9 lane 1), where the Smith's upgrade grid marks its row: `NDeckCardSelectScreen` holds the answer on the SCREEN, not in the grid's paint list. So the screen's own selection fields are unioned in, matched BY NAME (`select` / `chosen`) rather than by type — a screen holds `CardModel`s that are not a pick, and reporting the row under the cursor as armed is worse than the silence this replaces — cached per screen type, and never turning a null into a false. Read-only. |
