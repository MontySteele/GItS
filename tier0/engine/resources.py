...
def stage_retires_the_shipped_meters(player) -> bool:
    """`EB-745`. Does THE STAGE retire the shipped Encore and Fanfare meters
    for this player?

    THE FIND (round two, sec.4). "Fanfare 30 (buff), generated by losing HP,
    nothing spends it" sat in every seat's status list beside the bar of the
    same name, and one seat spent a card and an energy to learn which Fanfare
    <i>Ousia Surge</i> reads. Round one's `EB-736` hid the shipped METER LINE
    from the page; the machinery underneath it kept running, so the buff row
    came back through the status list and through the wire's resources.

    THE BRIEF RETIRES THEM OUTRIGHT (sec.2's table, R269): "Encore, the
    Spotlight in both modes, the Fanfare counter and its riders" have no job
    under the Stage, and sec.3 rule 11 says her own bars are touched by
...
def gain_fanfare(state: CombatState, n: int, source: str) -> None:
    ...
    if not p.fanfare_cap or n <= 0:
        return
    if stage_retires_the_shipped_meters(p):
        return                    # `EB-745`
    before = p.fanfare
