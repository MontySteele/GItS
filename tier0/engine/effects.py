...
def _runtime_count(state: CombatState, token: str,
    ...
    # QUARANTINED (`furina_stage.FURINA_STAGE`, `EB-730`) -- the STAGE's three.
    #
...
PREDICATE_NAMES = frozenset({
    ...
    "plan_held",
    # QUARANTINED (`furina_stage.FURINA_STAGE`, `EB-730`). THE ONE THING A
...
RUNTIME_COUNT_NAMES = frozenset({
    # QUARANTINED USE ONLY (`EB-730`) -- the FURINA STAGE's three. Registered
...
def _op_set_off(state: CombatState, fx: dict, card: Card) -> None:
    ...
    # ONCE MORE!'s NOTE (`EB-730`), taken HERE and above every branch below:
...
def _op_return_to_hand(state: CombatState, fx: dict, card: Card) -> None:
    """Blast Shield (`EB-730`): "Return this card to your hand."
...
def _op_return_last_set_off(state: CombatState, fx: dict, card: Card) -> None:
    """Once More! (`EB-730`): the last Set off card played this combat comes
...
OPS = {
    ...
    # QUARANTINED (`furina_stage.FURINA_STAGE`, `EB-730`): the Stage's eight.
...
def companion_overhaul_block_absorbed(state: CombatState, enemy: Enemy,
    ...
    `C.KLEE_OVERHAUL`, `EB-730`) IS FIRST AND HAS ITS OWN FLAG. Return to
