# Tier 0 -- Decision log, archive volume R39-R99

> **Lifecycle: ARCHIVED** -- frozen, append-only spine. VOLUME BANNER (Track
> R-D, "Clear the Stage" charter, SIGNED full-AUTHORIZE incl. R-D per R119;
> date 2026-08-06). Everything below the rule line is the **R39-R99 range of
> `tier0/DECISIONS.md`**, moved here **VERBATIM -- byte-identical, nothing
> rewritten (R101b)** -- including the interleaved D-series entries D2-D5 and
> every strike, banner and DRAFT marker inside the range, which travel with
> their rulings. Source: `tier0/DECISIONS.md` at `4179bd8` (origin/main,
> 2026-08-06). The live file keeps the pre-R39 prose record and R100+;
> R1-R38 were never written as headed entries anywhere and are NOT in this
> volume (queue: `docs/registry/user-queue.md` s.4). Resolve any R-number via
> `docs/registry/identifiers.md` s.3. One shared number sequence across
> volumes; renumbering is forbidden (charter s.5). This archive is a
> [USER]-gated register exactly like the live ledger: cite it, never write it.
> (identifier-registry: allow-bare -- frozen history restates its own tokens.)
