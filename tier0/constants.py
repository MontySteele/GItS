...
KURAGE_DURATION = 3           # turns the jellyfish holds the field. Stacks
                              # = turns remaining (the oz_summon grammar);
                              # re-summoning REFRESHES, never adds — a
                              # second jellyfish is not a bigger jellyfish.
KURAGE_PULSE_BASE = 2         # flat damage per turn-end pulse, before the
                              # bank read. Deliberately small: the pulse is
                              # an A2 signature, so it must be nearly
                              # worthless at Charge 0 and real by node 4.
KURAGE_PULSE_DIVISOR = 4      # pulse gains +1 damage per this much Charge.
                              # Coarser than the Garment's /2 on purpose:
                              # the summon ticks EVERY turn unattended,
                              # where the Garment is a bounded window that
                              # costs a card play to enter.
KURAGE_PULSE_BLOCK = 2        # Block granted by each pulse (party-wide in
                              # co-op). The healer fantasy under the R52
                              # healing law: her HP bar doesn't move, the
                              # party's incoming does.
