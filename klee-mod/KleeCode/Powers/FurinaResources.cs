...
public static class FurinaResources
{
    ...
    /// <summary>
    /// `EB-745`. DOES THE STAGE RETIRE THE SHIPPED ENCORE AND FANFARE METERS
    /// for this creature?
    ///
    /// THE FIND (round two, sec.4). "Fanfare 30 (buff), generated by losing
    /// HP, nothing spends it" sat in every seat's status list beside the BAR
    /// of the same name, and one seat spent a card and an energy to learn
    /// which Fanfare <i>Ousia Surge</i> reads. Round one's `EB-736` hid the
    /// shipped meter LINE from the blind page; the machinery underneath kept
    /// running, so the buff came back through the status list and through the
    /// wire's own resources block.
    ///
    /// THE BRIEF RETIRES THEM OUTRIGHT (sec.2's table, R269): Encore, the
    /// Spotlight in both modes and the Fanfare counter have no job under the
    ...
    ///
    /// Sim twin: <c>tier0.engine.resources.stage_retires_the_shipped_meters</c>.
    /// </summary>
    public static bool StageRetiresTheShippedMeters(Creature? creature)
    {
#if PROTOTYPE_CARDS
        return FurinaStage.LiveFor(creature);
#else
        return false;
#endif
    }
    ...
    {
        if (amount <= 0) return;
        if (StageRetiresTheShippedMeters(creature)) return;   // `EB-745`
        ...
        CardModel? cardSource = null)
    {
        ...
        // `EB-745`: under the Stage the shipped counter is retired, so its
        // BADGE is not synced either -- the mint gates above already keep the
        // number at 0, and a badge that reappears at 0 is exactly the row the
        // round-two seats filed ("Fanfare 30 (buff) ... nothing spends it").
        if (!StageRetiresTheShippedMeters(creature))
        {
...
public sealed class FurinaResourceHooks : AbstractModel
{
        ...
        CardModel? cardSource)
    {
        ...
        // hit emptied on HER turn takes its Bow here, after the hit is dealt
        // (rule 7); on the enemy's turn its Bow waits for the start of hers
        // (2026-09-25 evening, `FurinaStage.PayOwedBows`). The part of the
        // hit that reached HER is filed on the stage log first.
