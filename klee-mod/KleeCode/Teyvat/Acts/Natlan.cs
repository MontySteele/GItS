...
public sealed class Natlan : ActModel
{
    ...
    /// <summary>
    /// The Hive's own answer, epoch gate included. `Hive.GetUnlockedAncients`
    /// removes Orobas unless `OrobasEpoch` is revealed (`Hive.cs:104-111`);
    /// delegating carries that gate rather than restating it, so a dressed
    /// act 2 hides exactly the Ancient the base zone hides on the same save.
    /// </summary>
