...
| `SparkSinkPinTests.cs` | 16 | EB-118 §4.5's Spark sink: the `CanSpend` gate a generated sink hangs `IsPlayable` on (whole price or nothing), True Spark Knight's live threshold and its floor of 1, and two structural pins on `Spend` (it refuses through the same predicate the gate uses, and moves the bank through the same `PowerCmd.ModifyAmount` the threshold consume uses). No card prints the op. Plus the Sparks arm's flag read BOTH ways round -- this file is the only one that can say a RELEASE build still runs the base rule, because the file that says it is retired is not compiled without the switch. Same argument, second fact (`EB-281`): `SparkPower.SyncGauge` is the ONE quarantine seam the Spark gauge is reached through, and in a release build it therefore calls nothing at all. |
