"""Grade `KLEESPARK-W2`'s slate off a finished blind-play session.

COMMITTED BEFORE THE DEPLOY. Mechanical only. Every slot is computed from
...
batch of fights, not a comparison and not evidence about balance.
    ...
    python review/qa/blindplay/kleespark-w2/grade.py <log_dir> [--json out]

The slate is `review/active/klee-sparks-2026-08-29.md` sec 16.4:
  ...
  W4'  UNREACHED BY CONSTRUCTION (no redesigned price-3 rung was built).
       The two price-3 faces in the deck are counted, ungraded.
  W5'  the attack share -- RECORDED, NOT GRADED.

AFFORDABLE, once, for the whole slate (sec 16.4): printed Spark price <= the
printed Spark bank AND printed Energy cost <= the printed current Energy.
"""
...
# The granted deck, sec 16.2. Printed name -> Spark price.
DAMAGE_SINKS = {
    "Ka-pow!": 1,
    "Fwoosh!": 1,
    "Tinder Toss": 1,
...
# W3''s hold vocabulary, sec 14.4 condition 2. BOTH halves are required: the
# sentence must read as a deliberate keep AND must name a priced title or the
# bank itself, so a player who simply forgot the card in hand does not score
# as a player who held.
HOLD = re.compile(
...
def _pages(log_dir: Path) -> list[dict]:
    ...
    for prompt in sorted(log_dir.glob("turn-*/prompt.md")):
        ...
        # UNREAD ONE (`W1`'s grader, same reason): the game prints a power
        # only while it is held, so a spent-out bank prints nothing at all.
