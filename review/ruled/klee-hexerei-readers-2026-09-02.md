...
**Alice's Introduction Magic** (Rare, 1, Skill). *All cards in your hand
count as Hexerei cards.* Upgrade: Retain. Ruled by you in place of the
drafted Alice's Letters ("Hexerei cards cost 1 less" plus the element
override), which you read as too easy to take infinite once any Hexerei
card draws or makes Energy; the "first Hexerei card is free" form was
fairer but flat. This one is an enabler with a window: played first, every
card after it this turn is a witch's card, so Witches' Circle plants a
Bomb 3 per play, Coven Errand goes wide, Nicole's Ladder fires per card,
and the coven bonus lines on the Personals read as met. The name is the
Little Hexenzirkel's event spell. Two derived readings, APPLIED as D
defaults: the window is this turn, over the cards in hand when it is
played (a card drawn later this turn is not counted, so Retain on the
upgrade is the way to hold it for the big hand); and it counts as Hexerei
itself, so it does not need a second witch to start a circle.
...
Three rows in `docs/prototype-surface.yaml` under the Klee arm (`EB-326`), a
keyword tip for Hexerei on Klee's side ("Hexerei: a Companion card from the witches'
...
1. **How many.** RULED at the default. (1) *The three in §2* [default]. (2) Four, adding Hex and
   Wick now.
2. **Witches' Circle alone.** RULED at the default. (1) *Dead alone, as the brief's bridge card;
   the draft decides* [default]. (2) Add "When you play this, gain 1
   Spark" so it is never a blank.
3. **Alice's Letters.** RULED: replaced. [USER]: "'Alice's Letters' sounds
   insanely strong as-is (too easy to go infinite if you have any way to
   get card draw OR energy from the Hexerei cards); 'First Hexerei card is
   free' sounds a lot fairer, but I think we can make it more interesting.
   I would propose renaming it to 'Alice's Introduction Magic' after a
   Little Hexenzirkel event spell and changing the effect to a Skill that
   reads 'All cards in your hand count as Hexerei cards', cost 1 (upgrade
   to Retain)." The card in §2 is that card.
4. **When they are built.** RULED at the default. (1) *For round 8, after the round-7 seat read
