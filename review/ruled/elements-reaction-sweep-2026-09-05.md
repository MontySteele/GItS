...
## 6. The ruling (R263, 2026-09-06)

[USER] ruled all three picks in one message, on 2026-09-06, without the
local machine and with no playtest possible for two days. Read against §5:

1. **Dendro: option 1's timing with option 2's sourcing.** The
   implementation is deferred out of the immediate rebuild milestone (the
   three overhauls' Prototype loops and their Balance landings), but the
   design is drawn now, as boundaries, and **Sumeru is struck as a
   prerequisite.** [USER]'s new fact, stated in one line: the surface
   already carries cards for canonically Dendro characters that skip the
   Dendro part (Kirara, `proto_mi_kirara_surprise_dispatch`, declared
   element-less; Yaoyao, `proto_mc_yaoyao_yuegui_throwing_mode`, a Bomb
...
2. **The sweep: option 1, with a corrected baseline and a broader
   ...
   BUILT; `EB-410` stays the open half and is a bridge build); (ii) review
   the concrete sequences and the companion choices, as a READING and not a
   keyword count, because the census's largest bucket is "unmarked"
   (`review/records/reaction-census-2026-09-05.md` §1) and a count cannot
   say what a seat did; (iii) test a change only where a specific weakness
   is demonstrated by that reading. The reading is
   `review/records/reaction-sequences-2026-09-06.md`; the layer's brief is
   `review/active/reaction-brief-2026-09-06.md`. §4 item 4's three
   hypotheses are candidates, not a schedule: each waits on its weakness
   being shown in the reading before it becomes a round's hypothesis.
3. **Quicken: a version of option 1.** No LAW amendment; the iron rule is
   read, not changed. The bonus is **additive**, chosen so that Quicken is
   unique and does not compete with Superconduct: Superconduct's Vulnerable
   scales with the size of every hit, so Quicken's flat per-hit bonus must
   pay hit COUNT instead. The shape is drawn in the boundaries paper §4 and
   returns there as a pick with a marked default.

Nothing builds from this ruling. No register row opens; the two papers and
the reading are the work, and the R ceiling is 263. PR #397 is superseded by
the recording and closes unmerged.
