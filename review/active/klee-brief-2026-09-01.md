...
## 18. What LAW used to pin about Klee

Moved here verbatim from `docs/current/LAW.md` §*Character identity — Klee*
(lines 161 to 180) on 2026-09-01, by the machinery review's change 6:
character identity leaves LAW and lives in the character's brief, where it is
revised by a sentence rather than by a LAW amendment.

Recorded, not re-ratified. §3 above is this kit's live rule list. Where a line
below and §3 disagree, that disagreement is a design question for this brief to
settle, not a LAW conflict.
...
**What stayed in LAW:** the Personal-pool companion rules (Prune is named there
as an example, in the Economy section), the shared combat and element rules
every character runs on, and the engineering invariants. Klee's Spark bound
`KLEE_COMPANION_SPARK_*` is a countersigned LAW economy clause, not an identity
line, and did not move.
